...
<button class="search-btn" id="searchBtnNormal" onclick="doSearch()">开始检索</button>
<button class="search-btn" id="searchBtnLiuCun" onclick="doSearchLiuCun()" style="display:none; background:#8bc34a;">检索留存</button>
</div>
</div>
...
// ★★★ 往日今朝：纯文本合辑 + 一键复制 ★★★
function showJinRiHeJi() {{
    var today = new Date();
    var month = String(today.getMonth() + 1).padStart(2, '0');
    var day = String(today.getDate()).padStart(2, '0');
    var matched = POEMS.filter(function(p) {{
    ...
    if (matched.length === 0) {{
        c.innerHTML = '<p style="text-align:center;color:#888;margin-top:60px;">今日暂无历史诗词。</p>';
        return;
    }}
    ...
    var h = '<h3 style="margin-bottom:15px;color:#2e7d32;">📋 往日今朝（共' + matched.length + '首）</h3>';
    var fullText = '';
    ...
    h += '<button onclick="copyJinRiHeJi()" style="background:#4caf50; color:#fff; border:none; padding:8px 24px; border-radius:20px; cursor:pointer; font-size:0.9rem; letter-spacing:1px;">📋 一键复制全部</button>';
    h += '</div>';

    // 把纯文本内容存到全局变量，方便复制函数使用
    window._jinriheji_text = fullText;
...
// ★★★ 复制往日今朝全部内容 ★★★
function copyJinRiHeJi() {{
    var text = window._jinriheji_text || '';
    if (!text) return;
    if (navigator.clipboard) {{
        navigator.clipboard.writeText(text).then(function() {{
            var btn = document.querySelector('button[onclick="copyJinRiHeJi()"]');
            if (btn) {{
                btn.textContent = '✅ 已复制';
                setTimeout(function() {{ btn.textContent = '📋 一键复制全部'; }}, 1500);
            }}
        }}).catch(function() {{
            fallbackCopyJinRiHeJi(text);
        }});
    }} else {{
        fallbackCopyJinRiHeJi(text);
    }}
    function fallbackCopyJinRiHeJi(t) {{
        var dummy = document.createElement('textarea');
        document.body.appendChild(dummy);
        dummy.value = t;
        dummy.select();
...
// ★★★ 检索留存：打开弹窗（隐藏普通检索按钮，显示检索留存按钮） ★★★
function openJianSuoLiuCun() {{
    beforeCenterAction();
    ...
    // 隐藏普通“开始检索”按钮，显示“检索留存”按钮
    document.getElementById('searchBtnNormal').style.display = 'none';
    document.getElementById('searchBtnLiuCun').style.display = 'inline-block';
}}

// ★★★ 检索留存：执行检索并以合辑形式展示 ★★★
function doSearchLiuCun() {{
    const select = document.getElementById('searchGenreSelect');
    ...
    const c = document.getElementById('content');
    if (r.length === 0) {{
        c.innerHTML = '<p style="text-align:center;color:#888;margin-top:60px;">该检索条件下暂无诗词。</p>';
        return;
    }}

    let h = '<h3 style="margin-bottom:15px;color:#2e7d32;">📋 检索留存（共' + r.length + '首）</h3>';
    let fullText = '';
    ...
    h += '<div style="text-align:center; margin-top:10px;">';
    h += '<button onclick="copyLiuCunText()" style="background:#4caf50; color:#fff; border:none; padding:8px 24px; border-radius:20px; cursor:pointer; font-size:0.9rem; letter-spacing:1px;">📋 一键复制全部</button>';
    h += '</div>';

    window._liucun_text = fullText;
    c.innerHTML = h;
    c.scrollTop = 0;
...
// ★★★ 复制检索留存全部内容 ★★★
function copyLiuCunText() {{
    var text = window._liucun_text || '';
    if (!text) return;
    var btn = document.querySelector('button[onclick="copyLiuCunText()"]');
...
function openSearch() {{
    ...
    document.getElementById('searchEnd').value = '';
    document.getElementById('searchKeywords').value = '';
    // 显示普通“开始检索”按钮，隐藏“检索留存”按钮
    document.getElementById('searchBtnNormal').style.display = 'inline-block';
    document.getElementById('searchBtnLiuCun').style.display = 'none';
}}
